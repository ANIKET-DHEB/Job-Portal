...
const applyJob = async (req, res) => {
  try {
    ...
    try {
      ...
      console.log("JOB DETAILS ERROR:", jobError.message);
    }
          ...
          <div style="margin:0;padding:30px 15px;background:#f4f7fb;font-family:Arial,Helvetica,sans-serif;">
            <div style="max-width:600px;margin:auto;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 10px 30px rgba(0,0,0,0.08);">
              <div style="background:#2563eb;padding:25px;text-align:center;">
                <h1 style="margin:0;color:#ffffff;font-size:26px;">🎉 Application Received</h1>
              </div>
              <div style="padding:30px;">
                ...
                <p style="color:#475569;font-size:15px;line-height:1.7;">
                  Thank you for applying through our Job Portal. Your application has been successfully received.
                </p>
                <div style="margin:25px 0;padding:20px;background:#f8fafc;border-radius:12px;border:1px solid #e2e8f0;">
                  <p style="margin:8px 0;color:#334155;"><strong>Job:</strong> ${jobTitle}</p>
                  <p style="margin:8px 0;color:#334155;"><strong>Company:</strong> ${companyName}</p>
                  <p style="margin:8px 0;color:#334155;"><strong>Applicant:</strong> ${fullName}</p>
                  <p style="margin:8px 0;color:#334155;"><strong>Email:</strong> ${email}</p>
                </div>
                <p style="color:#475569;font-size:15px;line-height:1.7;">
                  Our team will review your application. If your profile is shortlisted, you will be contacted with the next steps.
                </p>
                <div style="margin-top:30px;padding-top:20px;border-top:1px solid #e2e8f0;">
                  <p style="margin:0;color:#64748b;font-size:14px;">Best regards,</p>
                  <p style="margin:5px 0 0;color:#2563eb;font-size:15px;font-weight:bold;">Job Portal Team</p>
                </div>
              </div>
            </div>
          </div>
...
const getApplicationById = async (req, res) => {
  try {
    ...
    if (
      req.user.role !== "admin" &&
      application.userId?._id.toString() !== req.user.id.toString()
    ) {
...
const updateApplicationStatus = async (req, res) => {
  try {
            ...
            <div style="margin:0;padding:30px 15px;background:#f4f7fb;font-family:Arial,Helvetica,sans-serif;">
              <div style="max-width:600px;margin:auto;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 10px 30px rgba(0,0,0,0.08);">
                <div style="background:#16a34a;padding:25px;text-align:center;">
                  <h1 style="margin:0;color:#ffffff;font-size:26px;">🎉 Congratulations!</h1>
                  <p style="margin:8px 0 0;color:#ffffff;font-size:16px;">Your application has been shortlisted</p>
                </div>
                <div style="padding:30px;">
                  <p style="margin:0 0 15px;color:#334155;font-size:16px;">
                  ...
                  <div style="margin:25px 0;padding:18px;background:#f8fafc;border-radius:10px;border:1px solid #e2e8f0;">
                    <p style="margin:0;color:#475569;font-size:14px;line-height:1.6;">
                      <strong>Important:</strong> Please keep checking your email and phone for further communication from our recruitment team.
                    </p>
                  </div>
                  <p style="color:#475569;font-size:15px;line-height:1.7;">
            ...
            <div style="margin:0;padding:30px 15px;background:#f4f7fb;font-family:Arial,Helvetica,sans-serif;">
              <div style="max-width:600px;margin:auto;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 10px 30px rgba(0,0,0,0.08);">
                <div style="background:#dc2626;padding:25px;text-align:center;">
                  <h1 style="margin:0;color:#ffffff;font-size:26px;">Application Update</h1>
                </div>
                <div style="padding:30px;">
                  <p style="margin:0 0 15px;color:#334155;font-size:16px;">
                    Hi <strong>${application.fullName}</strong>,
    ...
    if (status === "Shortlisted" && statusChanged) {
      responseMessage = "Application shortlisted and email sent successfully.";
    }

    if (status === "Rejected" && statusChanged) {
      responseMessage = "Application rejected and email sent successfully.";
    }

    if (!statusChanged) {
      responseMessage = "Application status is already " + status + ".";
    }
...
const deleteApplication = async (req, res) => {
  try {
    ...
    if (
      req.user.role !== "admin" &&
      application.userId.toString() !== req.user.id.toString()
    ) {
      return res.status(403).json({
        success: false,
